...
def delayMicrosecond(t):    # 微秒级延时函数
    ...
    t=(t-4)/1000000     # 将输入t的单位转换为秒，-3是时间补偿
...
def DHT11():
    ...
    GPIO.output(data,GPIO.HIGH) # 设置GPIO输出高电平
    delayMicrosecond(30)   # 延时30微秒 延时等待
    ...
    for i in range(40):         # 循环40次，接收温湿度数据
        ...
        delayMicrosecond(50)    # 延时30微秒
            
        if GPIO.input(data):    # 超过50微秒后判断是否还处于高电平
...
while True:
    ...
    if check==check_tmp and temperature!=0 and humidity!=0:  # 判断数据是否正常
